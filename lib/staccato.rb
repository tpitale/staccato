...
module Staccato
  ...
  def self.ga_collection_uri(ssl = false, debug = false)
    url_scheme = ssl ? 'https://' : 'http://'
    url_body = debug ? 'www.google-analytics.com/debug/collect' : 'www.google-analytics.com/collect'

    URI(url_scheme + url_body)
  end
